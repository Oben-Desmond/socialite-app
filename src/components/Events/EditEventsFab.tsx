...
const EditEventsFab: React.FC<{ post: PostInterface, comments: commentInterface[], onDelete: () => void, onEdit: () => void ,commentTitle:string}> = ({ post, onDelete, onEdit, comments,commentTitle }) => {
    ...
    return (
        <>
            ...
            <EditEventsModal commentTitle={commentTitle} post={post} onDidDismiss={() => seteditPost(false)} isOpen={editPost}></EditEventsModal>
        </>
    )
}


export default EditEventsFab
            ...
            <IonContent>
                ...
                <IonCardContent mode={`md`}>
                    <IonToolbar className={`ion-padding`} >
                        <IonCardTitle color={`secondary`} className={`ion-padding-start`}>Edit Event Post</IonCardTitle>
                    </IonToolbar >
                    <IonCardContent>
                        <form onSubmit={editPost} action="">
                            ...
                            <div ref={descRef} onClick={() => descRef.current?.scrollIntoView({ behavior: `smooth` })} style={{ whiteSpace: `pre-wrap` }} className="input">
                                <IonTextarea   rows={4}  value={input.description} onIonChange={e => setinput({ ...input, description: (e.detail.value || ``) })} required name={`description`} placeholder={`Enter post detail`}></IonTextarea>
                            </div>
                            {/* <div className={`input`}>
